...
void displayInotifyEvent(struct inotify_event *i)
{
    ...
    if ((i->mask & IN_MOVED_TO) && (i->mask & IN_ISDIR))
    {
        warnf("Adding %s to inotify FD.\n\n", i->name);
        nftw(path, nftwFunc, 20, flags); 
    }
    ...
    if (i->cookie > 0)
    {
        ...
        {
            cookie1 = i->cookie;
            errorf("Cookie 1 set: %d\n", cookie1);
        }
        else if(cookie2 == 0)
        {
            cookie2 = i->cookie;
            errorf("Cookie 2 set: %d\n", cookie2);
        }
        if(cookie1 == cookie2)
        {
...
int nftwFunc(const char *fpath, const struct stat *sb, int tflag, struct FTW *ftwbuf)
{
    ...
    {
        int wd = inotify_add_watch(inotifyFd, fpath, IN_CREATE|IN_DELETE|IN_DELETE_SELF|IN_MOVED_FROM|IN_MOVED_TO|IN_ISDIR|IN_MOVE_SELF);
        if (wd == -1)
        {
